        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4>إضافة مهمة جديدة لفريق: {{ $team->name }}</h4>
                    <a href="{{ route('manager.teams.tasks', $team) }}" class="btn btn-secondary">العودة</a>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('manager.teams.tasks.store', $team) }}">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="canceled" {{ old('status') == 'canceled' ? 'selected' : '' }}>ملغاة</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
